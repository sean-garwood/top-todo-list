...
import {
  todoListFormTemplate,
  todoItemFormTemplate,
} from "Constants/todo-forms";
...
const createTodoListElement = (todoList) => {
  ...
  const todoListElement = createTodoElement(
    "div",
    "todo-list",
    `
  ...
  (() => {
    const expandTodoListBtn = todoListElement.querySelector(
      ".expand-todo-list-btn",
    );
    const editTodoListBtn = todoListElement.querySelector(
      ".edit-todo-list-btn",
    );
    const deleteTodoListBtn = todoListElement.querySelector(
    ...
    function handleExpandClick(event) {
      function handleCollapseClick(event) {
        const detailsContainer = todoListElement.querySelector(
          ".todo-list-details-container",
        );
        detailsContainer.style.display =
          detailsContainer.style.display === "none" ? "block" : "none";
        event.target.textContent =
          event.target.textContent === "Expand Todo List"
            ? "Collapse Todo List"
            : "Expand Todo List";
...
const renderTodoLists = (todoLists) => {
  ...
  todoLists.forEach((todoList) => {
    ...
    renderTodoItems(
      todoList.todos,
      todoListElement.querySelector(".todo-items-container"),
    );
  });
